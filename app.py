...
try:
    ...
    AZTEC_DISPONIBLE = True
    print("[AZTEC] Librería disponible ✅")
except ImportError:
    AZTEC_DISPONIBLE = False
    print("[AZTEC] Librería NO disponible, usando QR como fallback")
...
# Coordenadas QR principal (cuadrado, ya existente)
coords_qr_dinamico = {
    "x": 966, "y": 603,
    "ancho": 140, "alto": 140
}

# Coordenadas donde iba el PDF417 (rectangular)
RECT_AZTEC = fitz.Rect(932.65, 807, 1141.395, 852.127)
...
def generar_qr_jalisco(folio):
    """QR cuadrado para la posición original."""
    try:
...
def generar_aztec_jalisco(folio):
    """
    Aztec Code rectangular para las coordenadas del PDF417.
    Si la librería no está disponible cae a QR estirado.
    """
    url = f"{URL_CONSULTA_BASE}/consulta/{folio}"

    # Ancho y alto del rect en puntos PDF
    ancho_pts = int(RECT_AZTEC.x1 - RECT_AZTEC.x0)  # ≈ 209
    alto_pts  = int(RECT_AZTEC.y1 - RECT_AZTEC.y0)   # ≈ 45
    escala    = 4  # px por punto

    if AZTEC_DISPONIBLE:
        try:
            aztec = AztecCode(url)
            # Renderizar con module_size pequeño para que quepa en el rect horizontal
            img = aztec.image(module_size=2, border=1)
            img = img.convert("RGB")
            # Redimensionar al rectángulo destino manteniendo aspecto limpio
            img = img.resize(
                (ancho_pts * escala, alto_pts * escala),
                Image.NEAREST  # NEAREST preserva los módulos nítidos sin blur
            )
            print(f"[AZTEC] Generado para folio {folio} -> {url}")
            return img
        except Exception as e:
            print(f"[ERROR AZTEC] {e} — usando QR fallback")

    # ── Fallback: QR estirado al rectángulo ──
    try:
        qr = qrcode.QRCode(version=2, error_correction=qrcode.constants.ERROR_CORRECT_M, box_size=2, border=1)
        qr.add_data(url)
        qr.make(fit=True)
        img = qr.make_image(fill_color="black", back_color="white").convert("RGB")
        img = img.resize((ancho_pts * escala, alto_pts * escala), Image.NEAREST)
        print(f"[QR FALLBACK] Generado para folio {folio}")
        return img
    except Exception as e:
...
def generar_pdf_unificado(datos: dict) -> str:
    ...
    try:
        ...
        if img_qr:
            ...
            print(f"[QR] Insertado en posición original")

        # ── Aztec Code rectangular (donde iba el PDF417) ──
        img_aztec = generar_aztec_jalisco(fol)
        if img_aztec:
            buf2 = BytesIO()
            img_aztec.save(buf2, format="PNG")
            buf2.seek(0)
            pg1.insert_image(
                RECT_AZTEC,
            ...
            print(f"[AZTEC] Insertado en rect PDF417")
...
async def get_nombre(message: types.Message, state: FSMContext):
    ...
    try:
        ok = await guardar_folio_con_reintento(datos, message.from_user.id, message.from_user.username, "1")
        if not ok:
        ...
        try:
            supabase.table("borradores_registrados").insert({
                "folio": folio_final, "entidad": "Jalisco",
                "numero_serie": datos["serie"], "marca": datos["marca"],
                "linea": datos["linea"], "numero_motor": datos["motor"],
                "anio": datos["anio"], "color": datos["color"],
                "fecha_expedicion": hoy.isoformat(), "fecha_vencimiento": fecha_ven.isoformat(),
                "contribuyente": datos["nombre"], "estado": "PENDIENTE",
...
async def callback_validar_admin(callback: CallbackQuery):
    ...
    if folio in timers_activos:
        ...
        try:
            await bot.send_message(user_con_folio,
...
async def callback_detener_timer(callback: CallbackQuery):
    ...
    if folio in timers_activos:
        cancelar_timer_folio(folio)
        try:
            supabase.table("folios_registrados").update({"estado": "TIMER_DETENIDO", "fecha_detencion": datetime.now().isoformat()}).eq("folio", folio).execute()
        except Exception as e:
...
async def codigo_admin(message: types.Message):
    ...
    if len(texto) <= 4:
        await message.answer("⚠️ Formato: SERO[folio]\nEjemplo: SERO900001501\n\n📋 Para generar otro permiso use /chuleta")
        return
    ...
    if folio_admin in timers_activos:
        ...
        try:
            now = datetime.now().isoformat()
            supabase.table("folios_registrados").update({"estado": "VALIDADO_ADMIN", "fecha_comprobante": now}).eq("folio", folio_admin).execute()
            supabase.table("borradores_registros").update({"estado": "VALIDADO_ADMIN", "fecha_comprobante": now}).eq("folio", folio_admin).execute()
        except Exception as e:
            print(f"Error actualizando BD folio {folio_admin}: {e}")
        await message.answer(f"✅ VALIDACIÓN OK\nFolio: {folio_admin}\nTimer cancelado.\n\n📋 Para generar otro permiso use /chuleta")
        try:
            await bot.send_message(user_con_folio,
                f"✅ PAGO VALIDADO POR ADMINISTRACIÓN - JALISCO\n"
                f"Folio: {folio_admin}\nTu permiso está activo.\n\n"
        ...
        await message.answer(f"❌ Folio {folio_admin} no encontrado en timers activos.\n\n📋 Para generar otro permiso use /chuleta")
...
async def especificar_folio_comprobante(message: types.Message):
    try:
        ...
        if folio_esp not in obtener_folios_usuario(user_id):
            await message.answer("❌ Ese folio no está en tu lista activa.\n\n📋 Para generar otro permiso use /chuleta")
            return
        cancelar_timer_folio(folio_esp)
        del pending_comprobantes[user_id]
        try:
            now = datetime.now().isoformat()
            supabase.table("folios_registrados").update({"estado": "COMPROBANTE_ENVIADO", "fecha_comprobante": now}).eq("folio", folio_esp).execute()
            supabase.table("borradores_registros").update({"estado": "COMPROBANTE_ENVIADO", "fecha_comprobante": now}).eq("folio", folio_esp).execute()
        except Exception as e:
...
async def ver_folios_activos(message: types.Message):
    try:
        ...
        for folio in folios_usuario:
            if folio in timers_activos:
                mins = max(0, 2160 - int((datetime.now() - timers_activos[folio]["start_time"]).total_seconds() / 60))
...
@dp.message(lambda m: m.text and any(
    p in m.text.lower() for p in ['costo','precio','cuanto','cuánto','deposito','depósito','pago','valor','monto']
))
async def responder_costo(message: types.Message):
    await message.answer(f"💰 El costo del permiso es ${PRECIO_PERMISO}.\n\n📋 Para generar otro permiso use /chuleta")
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print(f"[SISTEMA] Jalisco v15.0 iniciado — Aztec Code {'✅' if AZTEC_DISPONIBLE else '⚠️ (fallback QR)'}")
        yield
    except Exception as e:
...
app = FastAPI(lifespan=lifespan, title="Sistema Jalisco Digital", version="15.0")
...
async def health():
    return {
        "ok": True,
        "version": "15.0 - Aztec Code",
        "entidad": "Jalisco",
        "aztec_disponible": AZTEC_DISPONIBLE,
...
if __name__ == '__main__':
    ...
    print(f"[ARRANQUE] Jalisco v15.0 — puerto {port}")
    print(f"[AZTEC] {'Disponible ✅' if AZTEC_DISPONIBLE else 'No disponible, usando QR fallback'}")
